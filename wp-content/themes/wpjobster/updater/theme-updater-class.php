...
class EDD_Theme_Updater {
	...
	function __construct( $args = array(), $strings = array() ) {
		...
		add_action( 'admin_init',                          array( $this, 'load_themes_screen' ) );
	}
	...
	function update_nag() {
		...
		if ( defined( 'WPJ_DISABLE_UPDATES' ) && WPJ_DISABLE_UPDATES == true ) return;
		...
		if ( version_compare( $this->version, $api_response->new_version, '<' ) ) {
			do_action( 'wpj_before_update_nag', $api_response->new_version );
			...
			echo '</div>';
			do_action( 'wpj_after_update_nag' );
		}
	}
