...
Class ajax_login_register_Login Extends AjaxLogin {
	...
	public function login_submit( $user_login=null, $password=null, $is_ajax=true, $g_response=null ) {
		...
		if (get_option('ajax_login_register_user_email') == 'on') {
			...
			if ( validate_username( $username ) ){
				if ( username_exists( $username ) ){
					if ( get_option( 'wpjobster_enable_user_reCaptcha' ) == 'yes' && $validate_reCaptcha['code'] == 'error' ) {
						$status = $this->status('captcha_invalid');
					} else {
						$creds = array(
							'user_login'    => $username,
					...
					$status = $this->status('email_invalid');
				}
			} else {
				$status = $this->status('email_invalid');
			}
