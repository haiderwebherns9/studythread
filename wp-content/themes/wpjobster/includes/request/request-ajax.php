...
// ADD NEW REQUEST
				if (empty($rqq)) {
					...
					$subj_cat = get_post_meta($pid, 'req_subj_cat', true );
					// Check if the custom field has a value.
					if ( ! empty( $subj_cat ) ) {
						//print_r($subj_cat);exit;
						$sub_arr=explode(",",$subj_cat);
						$counter=0;
					foreach($sub_arr as $sval){
						//print_r($sval);
						$sq= "SELECT P.post_author FROM `wp_87fsrr_term_relationships`  AS TR
						LEFT JOIN `wp_87fsrr_posts` AS P ON P.ID=TR.object_id
						where TR.term_taxonomy_id=".$sval;
						$post_id = $wpdb->get_results($sq);
						 foreach($post_id as $k=>$val1){
							//print_r($val1->post_author);
							  $user_info = get_userdata($val1->post_author);
							  $username = $user_info->user_login;
							  $first_name = $user_info->first_name;
							  $last_name = $user_info->last_name;
							  $tmsg="Hired You, Please schedule the classes after the Bkash Payment have made to you.";
							  $smsg="Hey Thank you for selecting the time for the teacher ".$first_name." ".$last_name.", Your Classes will be scheduled after the Bkash Payment has been made to the Teacher.";
							  if($counter==0){
								   $wpdb->insert( 
