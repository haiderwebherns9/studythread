	...
	if ( $space_id == null ) {
		...
		if ((get_option('bsa_pro_plugin_' . 'hide_if_logged') == 'yes' && is_user_logged_in()) != true && // Hide for logged users
			...
			if (bsa_space($space_id, 'id') && bsa_space($space_id, 'status') == 'active' && $taxonomy == 'isset' ||
				...
				if (count($ads) > 0 || count($ads) == 0 && !isset($if_empty) && bsa_space($sid, 'display_type') != 'background' || $example ) {
...
function bsaProExpireSender ( $aid, $type )
{
	...
	$message = str_replace('[AD_ID]', $aid, $message);
...
function bsaProCountdown ( $sid, $aid, $ad_limit, $ad_model )
{
	...
	if ( $sid > 0 && $aid > 0 && $ad_limit > 0 ) {
		...
		$notice = 0;
		if ( $ad_model == 'cpd' ) { // cpd
			if ( $ad_limit < strtotime('-6 hours') && isset($getOpt['up_cpd_notice']) && $ad_limit > strtotime('-'.$getOpt['up_cpd_notice'].' days') && $getNotice != 1 ) { // less than X days and if not sent
				$notice = 1;
			} elseif ( $ad_limit >= strtotime('-6 hours') && $getNotice == 1 ) { // ad expired (less than 6 hours) and if not sent
				$notice = 2;
			}
		} elseif ( $ad_model == 'cpc' ) { // cpc
			if ( $ad_limit > 1 && isset($getOpt['up_cpc_notice']) && $ad_limit < $getOpt['up_cpc_notice'] && $getNotice != 1 ) { // less than X clicks and if not sent
				$notice = 1;
			} elseif ( $ad_limit <= 1 && $getNotice == 1 ) { // ad expired (less than 1 click) and if not sent
				$notice = 2;
			}
		} elseif ( $ad_model == 'cpm' ) { // cpm
			if ( $ad_limit > 5 && isset($getOpt['up_cpm_notice']) && $ad_limit < $getOpt['up_cpm_notice'] && $getNotice != 1 ) { // less than X views and if not sent
				$notice = 1;
		...
		if ( $notice == 1 || $notice == 2 ) {
				 ...
				 isset($getOpt['up_expired_notice']) && $getOpt['up_expired_notice'] == 'yes' && $notice == 2  ) // if expired notifications enabled
			{
				...
				$setNotice = bsa_parse_arr( bsa_ad($aid, 'additional'), 'push', array('notice' => ($notice == 2 ? 0 : 1)) ); // set notice at 1
