...
<div class="bsaProOrderingForm">
	...
	if ( isset($_GET['oid']) && $_GET['oid'] != '' && bsa_ad($_GET['oid'], 'id') != null && bsa_space(bsa_ad($_GET['oid'], 'space_id'), 'status') == 'active' && !isset($_GET['cid']) ||
		...
		if ( bsa_get_opt('settings', 'form_restrictions') == 'yes' && is_user_logged_in() || bsa_get_opt('settings', 'form_restrictions') != 'yes' ) {
		...
		';
				...
				echo "<strong>ADS Notice!</strong> Spaces are full or doesn't exist. Add new Space in Admin Panel.";
			}
		}
	}
echo '
